...
public class PickPhase {
	...
	private final BufferedImage[] preScaledRandom;
	...
	//@formatter:off
		...
		//plant backgrounds
		this.plantBackground = getScaledImage("/plant_select.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
		
		this.plantBackgroundText = getScaledImage("/plant_select_text.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);

		// fire backgrounds
		this.fireBackground = getScaledImage("/fire_select.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
	
		this.fireBackgroundText = getScaledImage("/fire_select_text.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
		
		// plant backgrounds 
		this.waterBackground = getScaledImage("/water_select.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
		
		this.waterBackgroundText = getScaledImage("/water_select_text.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
		
		//random Background
		
		this.randomBackground = getScaledImage("/random_select.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);
		
		this.randomBackgroundText = getScaledImage("/random_select_text.png",rectWidth, rectHeight, this.originalWidth, this.originalHeight);

		// pickphase_Confirm characters_prescaled for that purpose
		this.preScaledRandom = new BufferedImage []{getConfirmPhaseScaledImage(plantBackground),getConfirmPhaseScaledImage(plantBackgroundText), 
													getConfirmPhaseScaledImage(fireBackground),getConfirmPhaseScaledImage(fireBackgroundText),
													getConfirmPhaseScaledImage(waterBackground) , getConfirmPhaseScaledImage(waterBackgroundText)
													,getConfirmPhaseScaledImage(randomBackground), getConfirmPhaseScaledImage(randomBackgroundText)};
		
		this.proceedButton = new RoundRectangle2D.Double( ((this.game.getWidth() + preScaledRandom[0].getWidth() ) / 2 - this.game.getWidth() * 0.25),  (this.game.getHeight() / 2 )
				, (this.game.getWidth() * 0.15), (this.game.getHeight() * 0.09),4,4);
		
		this.returnToPickPhaseButton = new RoundRectangle2D.Double(((this.game.getWidth() + preScaledRandom[0].getWidth() ) / 2 + proceedButton.getWidth() * 0.3),
...
//		game.setGameState(GameState.Pickphase);
		...
		case Pickphase:
			...
			//drawText(g2d, "X", cancelButton, 40, Font.BOLD);
			
			
			g2d.setColor(Color.WHITE);
			g2d.setFont(new Font("Arial", Font.PLAIN, 40));
			g2d.drawString("select a character to proceed", (int) ((game.getWidth() - g.getFontMetrics().stringWidth("select a character to proceed"))/ 2  ) , (int) (game.getHeight() * 0.09));
		...
		case Pickphase_confirm:		
			...
			String tempSelectText = String.format(confirmText, random ? "random character" :this.playerType.toString());
			
			g2d.drawString(tempSelectText, (int) ((this.game.getWidth() + preScaledRandom[0].getWidth() ) / 2  - g2d.getFontMetrics().stringWidth(tempSelectText) / 2), (int) (game.getHeight() * 0.09));
			...
			g2d.setColor(Color.WHITE);
			drawText(g2d, "poceed", proceedButton.getBounds(), 40, Font.BOLD);
	...
	public void drawselectedImage(Graphics2D g2d, BufferedImage img1, BufferedImage img2, Stroke lineStroke,Color color) {
	...
	public void drawImages(Graphics2D g2d, BufferedImage image, BufferedImage image_text, Stroke stroke, Color color,Rectangle rect) {
			...
			switch(game.getGameState()) {
				case Pickphase:
					if (Character1.getBounds().contains(mousePos)) {
						...
						playerType = randomValue == 0 || randomValue == 1 ? Type.plant : randomValue == 2 || randomValue == 3 ? Type.fire : Type.water;
	...
	private BufferedImage getScaledImage(String path, double desiredWidth, double desiredHeight, double originalWidth, double originalHeight) {
		return this.game.getImageLoader().loadImg(path).getScaledImage(desiredWidth, desiredHeight, originalWidth, originalHeight);
	}
